...
- 문제를 이해하는 데 중요한 부분:
- 레이저와 쇠막대기의 구분이 핵심.
- ()로 표시된 레이저는 쇠막대기의 잘리는 부분을 의미하고, 스택을 활용하여 열리고 닫히는 괄호를 관리하며 레이저로 쇠막대기가 분리될 때마다 그 개수를 세는 방식으로 해결.

## ⏱️ 코드의 시간복잡도

- 시간복잡도: O(N) (N은 입력 문자열의 길이)
- 각 괄호를 순회하면서 스택에 push 또는 pop을 수행하는 방식으로 문제를 해결.

## 🧠 코드의 공간복잡도

- 공간복잡도: O(N)
- 스택에 최대 N개의 괄호를 저장할 수 있으므로, 공간 복잡도는 N에 비례.
